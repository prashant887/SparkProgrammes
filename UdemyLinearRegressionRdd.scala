...
object UdemyLinearRegressionRdd {

  def main(args: Array[String]): Unit = {

    Logger.getLogger("org").setLevel(Level.ERROR)

    val sc=new SparkContext("local[*]","UdemyLinearRegressionRdd")
    ...
    val data=sc.textFile("D:\\SparkScalaUdemy\\regression.txt")
    ...
    val traingData=data.map(x=>{LabeledPoint.parse(x)}).cache()
    val testData=data.map(x=>{LabeledPoint.parse(x)})
    ...
    val model=algorithm.run(traingData)

    // Predict values for our test feature data using our linear regression model
val prediction=model.predict(testData.map(x=>{x.features}))

    // Zip in the "real" values so we can compare them
val predictionLabel=prediction.zip(testData.map(x=>{x.label}))

    for (predict <- predictionLabel){
      println(predict)
    }
  }
}
