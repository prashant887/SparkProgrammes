...
object DegreeOfSeperation {
  ...
  def createStartingRdd(sc:SparkContext): RDD[BFSNode] = {
    val inputFile = sc.textFile("D:\\SparkScalaUdemy\\Marvel-graph.txt")
    return inputFile.map(x=>{convertToBFS(x)})
  }
  ...
  def main(args: Array[String]) {
    ...
    for (iteration <- 1 to 10) {
      ...
      val mapped = iterationRdd.flatMap(x=>{bfsMap(x)})
      ...
      iterationRdd = mapped.reduceByKey((x,y)=>{bfsReduce(x,y)})
    }
  }
}